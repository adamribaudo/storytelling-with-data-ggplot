...
df <- read_csv("data\\FIG0206-7.csv") %>% 
  ...
  mutate(color = if_else(`Cost Per Mile` < mean(`Cost Per Mile`), GRAY9,ORANGE1))

avg_miles <- mean(df$`Miles Driven`)
avg_cost <- mean(df$`Cost Per Mile`)

pt <- ggplot(df) + geom_point(aes(x=`Miles Driven`,y=`Cost Per Mile`, color = color), size = 2.5) +
...
ggsave("plot output\\FIG0207.png", pt, width = 4.5, height = 4.2)
pt
